...
class SupplyChainDatabase:
    ...
    def _row_to_package_version(self, row: sqlite3.Row) -> PackageVersion:
        ...
        return PackageVersion(
            ...
            author=row['author'],
            author_email=row['author_email'] if 'author_email' in row.keys() and row['author_email'] else None,
            published_at=datetime.fromisoformat(row['published_at']) if row['published_at'] else None,
            processed_at=datetime.fromisoformat(row['processed_at']) if 'processed_at' in row.keys() and row['processed_at'] else None,
            ...
            homepage=row['homepage'] if 'homepage' in row.keys() and row['homepage'] else None,
            repository_url=row['repository_url'] if 'repository_url' in row.keys() and row['repository_url'] else None,
            license=row['license'] if 'license' in row.keys() and row['license'] else None,
            dependencies=json.loads(row['dependencies']) if 'dependencies' in row.keys() and row['dependencies'] else {},
            dev_dependencies=json.loads(row['dev_dependencies']) if 'dev_dependencies' in row.keys() and row['dev_dependencies'] else {},
            maintainers=json.loads(row['maintainers']) if 'maintainers' in row.keys() and row['maintainers'] else [],
            file_count=row['file_count'] if 'file_count' in row.keys() and row['file_count'] else None,
            unpack_size=row['unpack_size'] if 'unpack_size' in row.keys() and row['unpack_size'] else None,
            tarball_size=row['tarball_size'] if 'tarball_size' in row.keys() and row['tarball_size'] else None,
            created_at=row['created_at'] if 'created_at' in row.keys() and row['created_at'] else None,
            is_deprecated=bool(row['is_deprecated']) if 'is_deprecated' in row.keys() and row['is_deprecated'] else False,
            deprecated_reason=row['deprecated_reason'] if 'deprecated_reason' in row.keys() and row['deprecated_reason'] else None,
            dist_tags=json.loads(row['dist_tags']) if 'dist_tags' in row.keys() and row['dist_tags'] else {},
            shasum=row['shasum'] if 'shasum' in row.keys() and row['shasum'] else None,
            integrity=row['integrity'] if 'integrity' in row.keys() and row['integrity'] else None,
            git_head=row['git_head'] if 'git_head' in row.keys() and row['git_head'] else None
        )
